...
def about():
       print(''' Ritik goyal
                 ...
                 E- mail :- goyalritik051@gmail.com''')
       final()
def titlebar():
        print('-'*79)
...
def wordpress():
    ...
    os.system('docker run -dit -e MYSQL_ROOT_PASSWORD=ritik -e MYSQL_USER=Ritik -e MYSQL_PASSWORD=ritik -e MYSQL_DATABASE=my_database mysql:5.7')
    os.system(''' echo """
...
services:
        data_base_os:
                ...
                restart: always
                environment:
                        MYSQL_ROOT_PASSWORD: ritik
                        MYSQL_USER: Ritik
                        MYSQL_PASSWORD: ritik
...
def httpd_configure_always():
    ...
    os.system('docker container run  -dit --name  ritik707054   {0}'.format(image))
    check_httpd_installed_or_not=os.system('docker container exec ritik707054 rpm -q httpd')
    os.system('docker container rm -f ritik707054')
...
os.system( 'echo /usr/sbin/httpd  >> /root/.bashrc')
        ...
        os.system('wait $!')
        os.system('docker cp ritik123.py {}:/root/ritik123.py '.format(name))
        os.system('wait $!')
        os.system('docker exec {0} /usr/sbin/httpd'.format(name))
        os.system('wait $!')
        os.system('docker exec {0} python /root/ritik123.py'.format(name))
        os.system('wait $!')
        if choice == 'docker':
            ip_container(name)
        else:
...
def upload():
        ...
        if '/' not in select :
          print('you need to rename your image like :  ritik/webserver:v1')
